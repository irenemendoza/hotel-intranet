...
class MaintenanceRequestListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = (
            super().get_queryset().select_related("room", "reported_by", "assigned_to")
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["stats"] = {
            "pending": all_requests.filter(
                status=MaintenanceTask.StatusChoices.PENDING
            ).count(),
            "in_progress": all_requests.filter(
                status=MaintenanceTask.StatusChoices.IN_PROGRESS
            ).count(),
            "urgent": all_requests.filter(
                priority=MaintenanceTask.PriorityChoices.URGENT
            ).count(),
        }
...
class MaintenanceRequestUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, "Solicitud de mantenimiento actualizada correctamente."
        )
