...
class RoomDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if total_rooms > 0:
            context["stats"]["occupancy_rate"] = round(
                (context["stats"]["occupied"] / total_rooms) * 100, 1
            )
            context["stats"]["clean_rate"] = round(
                (context["stats"]["clean"] / total_rooms) * 100, 1
            )
        else:
        ...
        # Tareas pendientes
        context["pending_cleanings"] = (
            CleaningTask.objects.filter(
                status__in=[
                    CleaningTask.StatusChoices.PENDING,
                    CleaningTask.StatusChoices.IN_PROGRESS,
                ]
            )
        ...
        context["pending_maintenance"] = (
            MaintenanceTask.objects.filter(
                status__in=[
                    MaintenanceTask.StatusChoices.PENDING,
...
class RoomTypeCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request,
            f'Tipo de habitación "{form.instance.name}" creado exitosamente.',
        )
...
class RoomTypeUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request,
            f'Tipo de habitación "{form.instance.name}" actualizado exitosamente.',
        )
        return super().form_valid(form)
...
class RoomTypeDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        try:
            # Django verificará automáticamente con PROTECT
            response = super().delete(request, *args, **kwargs)
            messages.success(
                request,
                f'Tipo de habitación "{room_type.name}" eliminado exitosamente.',
            )
            return response

        except ProtectedError:
            # Django lanzó el error automáticamente
            messages.error(
                request,
...
class RoomDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["cleaning_history"] = self.object.cleaning_tasks.all().order_by(
            "-created_at"
        )[:10]
        ...
        context[
            "maintenance_history"
        ] = self.object.maintenance_requests.all().order_by("-created_at")[:10]
...
class RoomCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f"Habitación {form.instance.number} creada exitosamente."
        )
        return super().form_valid(form)
...
class RoomUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f"Habitación {form.instance.number} actualizada exitosamente."
        )
        return super().form_valid(form)
